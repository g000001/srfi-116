...
(define (check-arg pred val caller)
  (let lp ((val val))
    (if (_pred val) val (lp (error "Bad argument" val pred caller)))))
...
(define (%cdrs lists)
  ...
  (cl:block abort
	...
	(if (pair? lists)
	    (let ((lis (car lists)))
	      (if (null? lis) (cl:return-from abort '())
		  (cons (icdr lis) (recur (cdr lists)))))
	    '()))))
  ...
  (cl:block abort
    (let recur ((ilists ilists))
        (if (pair? ilists)
            (let ((ilist (car ilists))
                  (other-ilists (cdr ilists)))
	      (if (null? ilist) (cl:return-from abort
...
(define (icount pred ilist1 . ilists)
  ...
  (if (pair? ilists)
	...
	(if (null-ilist? lis) i
	    (lp (icdr lis) (if (_pred (icar lis)) (+ i 1) i))))))
...
(define (iunfold p f g seed . maybe-tail-gen)
  ...
  (if (pair? maybe-tail-gen)
	...
	(if (pair? (cdr maybe-tail-gen))
	    ...
	    (let recur ((seed seed))
	      (if (_p seed) (_tail-gen seed)
		  (ipair (_f seed) (recur (_g seed)))))))
...
(define (ifold kons knil ilis1 . ilists)
  ...
  (if (pair? ilists)
	...
	(if (null-ilist? ilis) ans
	    (lp (icdr ilis) (_kons (icar ilis) ans))))))
...
(define (ipair-fold-right f zero ilis1 . ilists)
  ...
  (if (pair? ilists)
	...
	(let ((cdrs (%cdrs ilists)))
      ...
      (let recur ((ilis ilis1))				; Fast path
	(if (null-ilist? ilis) zero (_f ilis (recur (icdr ilis)))))))
  ...
  (if (pair? ilists)
	...
	(if (null-ilist? ilis) ans
	    (let ((tail (icdr ilis)))		; Grab the icdr now,
	      (lp tail (_f ilis ans)))))))	; in case F SET-CDR!s LIS.
...
(define (ireduce-right f ridentity ilis)
  ...
  (if (null-ilist? ilis) ridentity
	...
	(if (ipair? ilis)
	    (_f head (recur (icar ilis) (icdr ilis)))
	    head))))
  ...
  (if (pair? ilists)
	...
	(if (null? cars) '()
		...
		(receive (cars2 cdrs2) (%cars+cdrs cdrs)
		  (if (null? cars2) vals
		      (_appender vals (recur cars2 cdrs2))))))))
	  ...
	  (let recur ((elt (icar ilis1)) (rest (icdr ilis1)))
	    (let ((vals (_f elt)))
		  ...
		  (_appender vals (recur (icar rest) (icdr rest)))))))))
...
(define (ipair-for-each proc ilis1 . ilists)
  ...
  (if (pair? ilists)
	...
	(if (not (null-ilist? ilis))
	    (let ((tail (icdr ilis)))	; Grab the icdr now,
	      (_proc ilis)		; even though nothing can happen
	      (lp tail))))))
  ...
  (if (pair? ilists)
	...
	(if (null-ilist? ilis) ilis
	    (let ((tail (recur (icdr ilis))))
	      (cond ((_f (icar ilis)) => (lambda (x) (ipair x tail)))
		    (else tail)))))))
...
;;; Inline us, please.
(define (iremove  pred l) (ifilter  (lambda (x) (not (_pred x))) l))
...
(define (idelete x lis . maybe-=) 
  (let ((= (\:optional maybe-= #'equal?)))
    (ifilter (lambda (y) (not (_= x y))) lis)))

;;; Extended from R4RS to take an optional comparison argument.
(define (imember x lis . maybe-=)
  (let ((= (\:optional maybe-= #'equal?)))
    (ifind-tail (lambda (y) (_= x y)) lis)))
...
(define (iassoc x lis . maybe-=)
  (let ((= (\:optional maybe-= #'equal?)))
    (ifind (lambda (entry) (_= x (icar entry))) lis)))
...
(define (ialist-delete key alist . maybe-=)
  (let ((= (\:optional maybe-= #'equal?)))
    (ifilter (lambda (elt) (not (_= key (icar elt)))) alist)))
...
(define (ibreak  pred lis) (ispan  (lambda (x) (not (_pred x))) lis))
  ...
  (if (ipair? lists)
	...
	(or (not (ipair? heads))
	  ...
	  (let lp ((head (icar lis1))  (tail (icdr lis1)))
	    (if (null-ilist? tail)
		(_pred head)	; Last PRED app is tail call.
		(and (_pred head) (lp (icar tail) (icdr tail))))))))
  ...
  (if (pair? ilists)
      ...
      (and (not (null-ilist? ilis1))
           (let lp ((head (icar ilis1)) (tail (icdr ilis1)))
             (if (null-ilist? tail)
                 (_pred head)            ; Last PRED app is tail call.
                 (or (_pred head) (lp (icar tail) (icdr tail))))))))
...
(define (ilist-index pred lis1 . lists)
  ...
  (if (pair? lists)
	...
	(and (not (null-ilist? lis))
	     (if (_pred (icar lis)) n (lp (icdr lis) (+ n 1)))))))
