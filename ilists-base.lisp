...
(define (ipair? obj)
  (if (null? (%ipair? obj)) #f #t))


(cl:defmethod cl:print-object ((obj <ilist>) stream)
  (if (proper-ilist? obj)
      (cl:format stream
                 "#,(ilist ~{~S~^ ~})"
                 (ilist->list obj))
...
(define-syntax receive
  (syntax-rules ()
    ((receive formals expression body cl:***)
     (call-with-values (lambda () expression)
                       (lambda formals body cl:***)))))
      ...
      (let lp ((lis lis1))
        (if (not (null-ilist? lis))
            (let ((tail (icdr lis)))    ; Grab the icdr now,
              (_proc (icar lis))                ; even though it's unnecessary
              (lp tail))))))
